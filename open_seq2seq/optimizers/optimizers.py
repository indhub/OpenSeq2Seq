...
class ApplyGradients():
  def __init__(self, opt, grads_and_vars, global_step, name, block_end=False):
    self._opt = opt
          ...
          with tf.name_scope("bmuf"):
            for grad, var in self._grads_and_vars:
              if grad is not None:
                ...
                prev_weight = self._get_or_make_slot(var, var.initialized_value(), 'prev_weight', self._name)
                prev_delta = self._zeros_slot(var, "prev_delta", self._name)

                G_t = var_avg - prev_weight
                curr_delta = block_momentum * prev_delta + block_lr * G_t

                # NBM scheme
                var_avg = prev_weight + curr_delta + block_momentum * curr_delta

                assigns.append(prev_weight.assign(var_avg))
                assigns.append(prev_delta.assign(curr_delta))
                assigns.append(var.assign(var_avg))
      return tf.group(*assigns)


class BMUFOptimizer(tf.train.Optimizer):
  def __init__(self, optimizer, name=None, use_locking=False, device_dense='',
               device_sparse='', bmuf_every=8):
    if name is None:
